  ...
  return (
    <div className="min-h-screen bg-white md:ml-24 animate-fadeIn">
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-8 border-t border-gray-50">
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-gray-400 mb-2"><Phone size={14} /><span className="text-[10px] font-bold uppercase tracking-widest">Customer Center</span></div>
                  <p className="text-xl font-black tracking-tighter">{info.callCenter}</p>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-gray-400 mb-2"><Mail size={14} /><span className="text-[10px] font-bold uppercase tracking-widest">Email Inquiry</span></div>
          ...
          <div className="lg:col-span-7">
            <div className="relative group">
              <div className="aspect-[4/5] md:aspect-square bg-gray-100 overflow-hidden relative shadow-2xl border border-gray-100">
                <iframe title="JYDESIGN Location Map" src={mapIframeUrl} className="w-full h-full grayscale-[1] contrast-[1.1] brightness-[0.7] group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-700 ease-in-out pointer-events-auto" style={{ border: 0 }} allowFullScreen={true} loading="lazy"></iframe>
                ...
                <div className="absolute bottom-10 left-10 right-10 bg-white p-8 md:p-12 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-700 pointer-events-auto">
                  <div className="flex items-start gap-4">
                    <div className="bg-black p-3 rounded-full text-white"><MapPin size={24} /></div>
                    <div><h4 className="text-lg font-bold tracking-tighter mb-1">Naver Maps</h4><p className="text-xs text-gray-400">네이버 지도로 정확한 위치를 확인하세요.</p></div>
                  </div>
                  <a href={info.mapUrl || "https://map.naver.com"} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-8 py-4 bg-black text-white text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-neutral-800 transition-all group/btn shadow-lg">Open Map <ExternalLink size={14} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" /></a>
                </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-gray-50 text-center">
        <h2 className="text-[12vw] font-black text-white leading-none tracking-tighter select-none mb-12">JYDESIGN</h2>
        <p className="text-[10px] font-bold tracking-[0.8em] text-gray-300 uppercase">Beyond the Space, Design for Life</p>
      </section>
    </div>
  );
};
